...
export default function Basket(){
    ...
    function deleteToBasket(id){
        const updateBasket = basket.map(item => {
            return item.id === id ? {...item, quantity: (item.quantity > 1) ? item.quantity - 1 : item.quantity} : item;
          })
                            ...
                            (!basket.length) ? false : basket.map(items => (
                                <div className="product" key={items.id}>
                                    ...
                                    <div className="quantity-block">
                                        <button onClick={() => deleteToBasket(items.id)} style={{borderRadius: "6px 0 0 6px"}}>-</button>
                                        <p>{items.quantity}</p>
                                        <button onClick={() => addToBasket(items.id)} style={{borderRadius: "0 6px 6px 0"}}>+</button>
                                    </div>
                                    <h4>{items.price} ₽</h4>
                                    <CloseIcon className="cross" onClick={() => deleteBasket(items)} />
                                </div>
                            ))
                        }
                </main>
